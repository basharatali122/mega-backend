...
class Database {
  ...
  generateAccounts(username, startRange, endRange, password) {
    // Only pad if startRange itself has leading zeros (e.g. user typed "001").
    // Use the digit length of startRange as the pad width so that
    // start=1, end=1000 → Bash1…Bash1000  (no extra zeros)
    // start=001, end=100 → Bash001…Bash100 (preserves intended padding)
    const pad = String(startRange).length;
    const rows = [];
    for (let i = startRange; i <= endRange; i++) {
      rows.push({ username: `${username}${String(i).padStart(pad, '0')}`, password, score: 0 });
    }
