...
/**
 * regular-wheel-processor.js  —  MEGASPIN WHEEL BOT
 *
 * Exact flow (reverse-engineered from browser capture):
 *
 *  CONNECTION 1 — login + availability check:
 *    → subID:6   login (account + password)
 *    ← subID:116 session data (userid, dynamicpass, bossid)
 *    → subID:11  game list ping
 *    → subID:10  bossid ping
 *    → subID:26  check wheel (userid + password)
 *    ← subID:142 blottery:1=available / blottery:0=already spun
 *
 *  If blottery=0 → skip spin, mark as "already done"
 *
 *  CONNECTION 2 — spin only (only opened when blottery=1):
 *    → subID:16  spin (userid + dynamicpass)
 *    ← subID:131 result (lotteryscore, score)
 *
 * Login result codes:
 *   0  = success
 *   1  = wrong password     → bad account, NEVER retry
 *   2  = account not found  → bad account, NEVER retry
 *   3  = wrong pass variant → bad account, NEVER retry
 *  -1  = server error       → may retry once
 *
 * WORKERS default = 10 (safe for Render free tier: 512MB RAM)
 * For local PC or paid VPS, set WORKERS = 50 for full throughput.
 */
...
const SERVER_REJECT_CODES = new Set([1, 2, 3]);
...
class RegularWheelProcessor extends EventEmitter {
  constructor(db) {
    ...
    this.config = {
      ...
      // 10 = safe for Render free tier (10 workers x 2 connections = 20 concurrent WS)
      // 50 = for local PC or VPS deployment
      WORKERS:        10,
      STAGGER_MS:     100,
      RETRY_ATTEMPTS: 1,
  ...
  async startProcessing(accountIds, repetitions = 1, useProxy = false, proxyList = []) {
    ...
    this.stats = {
      successCount: 0, failCount: 0, wheelSpins: 0, alreadySpun: 0,
      badAccounts: 0, totalScoreWon: 0, activeWorkers: 0, processed: 0,
      startTime: Date.now(),
    };
    ...
    this.currentAccounts = all.filter(a => accountIds.includes(a.id));
    const total = this.currentAccounts.length;
    const estHourly = Math.round(workerCount * (3600 / 3.5));
    ...
    this._emit('terminal', { type: 'info', message: '⚠️  "Already done" = account spun today. Use fresh accounts for new spins.' });
    this._emit('status',   { running: true, total, current: 0, activeWorkers: 0 });
    ...
    const worker = async () => {
      while (this.isProcessing) {
        ...
        if (this.stats.processed % 25 === 0 || this.stats.processed === total) {
          ...
          this._emit('terminal', {
            type: 'info',
            message: `📊 ${this.stats.processed}/${total} | 🎡${this.stats.wheelSpins} spun | ⏭️${this.stats.alreadySpun} done | 🚫${this.stats.badAccounts} bad | ❌${this.stats.failCount} err | ⚡${rate.toLocaleString()}/hr`,
          });
        }
      }
    };
  ...
  async _processWithRetry(account, globalIndex, attempt = 0) {
    ...
    let logStatus = 'error';
    if (result.success)         logStatus = result.wheelSpun ? 'spun' : 'already_spun';
    else if (result.badAccount) logStatus = 'bad_account';
    else if (result.serverError)logStatus = 'server_error';

    await this.db.addProcessingLog(
      account.id, logStatus,
      result.success
        ? (result.wheelSpun ? `Spun +${result.lotteryscore || 0}` : 'Already spun today')
        : result.error,
      result
    ).catch(() => {});

    // Bad account (wrong password / not found) — never retry, just count and skip
    if (result.badAccount) {
      this.stats.badAccounts++;
      return result;
    }

    // Already spun — valid completed state
    if (result.success && !result.wheelSpun) {
      this.stats.successCount++;
      this.stats.alreadySpun++;
      return result;
    }
  ...
  async _accountFlow(account, index) {
    ...
    try {
      ...
      return { success: false, error: `Conn1: ${err.message}` };
    }

    if (!loginResult.success) return loginResult;

    if (!loginResult.available) {
      return { success: true, wheelSpun: false, lotteryscore: 0, newScore: loginResult.score };
    }
    ...
    try {
      ...
      return { success: false, error: `Conn2: ${err.message}` };
    }
  ...
  _loginAndCheck(account, index, proxyUrl) {
    return new Promise(async (resolve) => {
      let ws = null, phase = 'login', closed = false;
      let userid = null, dynamicpass = null, bossid = null;
      let score  = account.score || 0;

      const timer = setTimeout(() => {
        close();
        resolve({ success: false, error: 'Conn1 timeout', networkError: true });
      }, this.config.TIMEOUTS.CONN1);
      ...
      const done = (result) => {
        if (phase === 'done') return;
        phase = 'done'; close(); resolve(result);
      };
      ...
      try {
        ...
        clearTimeout(timer);
        return resolve({ success: false, error: `WS: ${err.message}`, networkError: true });
      }
      ...
      ws.on('message', (raw) => {
        ...
        if (msg.subID === 116 && phase === 'login') {
          ...
          if (SERVER_REJECT_CODES.has(d.result) || !d.userid) {
            return done({ success: false, badAccount: true, resultCode: d.result, error: `Bad account result=${d.result}` });
          }
          if (d.result === -1) {
            return done({ success: false, serverError: true, error: 'Server error result=-1' });
          }

          userid = d.userid; dynamicpass = d.dynamicpass; bossid = d.bossid;
          score  = d.score !== undefined ? d.score : score;
          account.userid = userid; account.dynamicpass = dynamicpass; account.bossid = bossid;

          phase = 'check';
          ws.send(JSON.stringify({ userid, mainID: 100, subID: 11 }));
          ws.send(JSON.stringify({ bossid, mainID: 100, subID: 10 }));
          ws.send(JSON.stringify({ userid, password: account.password, mainID: 100, subID: 26 }));
          return;
        }

        if (msg.subID === 142 && phase === 'check') {
          const d = msg.data || {};
          if (d.dynamicpass) { dynamicpass = d.dynamicpass; account.dynamicpass = dynamicpass; }
          if (d.score !== undefined) score = d.score;

          const available = d.blottery === 1;
          if (!available) this._log(index, 'info', `⏭️ Already spun (resets: ${d.refreshtime || '?'})`);

          return done({ success: true, available, userid, dynamicpass, score });
        }
      });

      ws.on('error', (err) => done({ success: false, error: err.message, networkError: true }));
      ws.on('close', () => { if (phase !== 'done') done({ success: false, error: 'Conn1 closed early', networkError: true }); });
    });
  }
  ...
  _spinWheel(account, index, proxyUrl, loginResult) {
    return new Promise(async (resolve) => {
      let ws = null, phase = 'spin', closed = false;
      const { userid, dynamicpass, score: checkScore } = loginResult;
      let score = checkScore || account.score || 0;

      const timer = setTimeout(() => {
        close();
        resolve({ success: false, error: 'Spin timeout', networkError: true });
      }, this.config.TIMEOUTS.CONN2);
      ...
      try {
        ...
        clearTimeout(timer);
        return resolve({ success: false, error: `Spin WS: ${err.message}`, networkError: true });
      }

      ws.on('open', () => {
        ws.send(JSON.stringify({ userid, dynamicpass, mainID: 100, subID: 16 }));
      });
      ...
      ws.on('message', (raw) => {
        ...
        if (msg.subID === 131) {
          ...
          if (d.result === 0) {
            this._log(index, 'success', `🎉 +${lotteryscore} | bal:${newScore}`);
            return done({ success: true, wheelSpun: true, lotteryscore, newScore });
          }

          this._log(index, 'error', `❌ Spin rejected result=${d.result}`);
          return done({
            success: false,
            badAccount:  SERVER_REJECT_CODES.has(d.result),
            serverError: d.result === -1,
            error: `Spin rejected result=${d.result}`,
            resultCode: d.result,
          });
        }
      });
      ...
      ws.on('error', (err) => {
        if (phase !== 'done') { phase = 'done'; close(); resolve({ success: false, error: `Spin WS: ${err.message}`, networkError: true }); }
      });
      ws.on('close', () => {
        if (phase !== 'done') { phase = 'done'; close(); resolve({ success: false, error: 'Spin closed early', networkError: true }); }
      });
    });
  }
  ...
  _complete() {
    ...
    this._emit('terminal', { type: 'success', message: '\n🎉 ALL DONE!' });
    this._emit('terminal', { type: 'info', message: `🎡 Spun: ${this.stats.wheelSpins} | ⏭️ Already done: ${this.stats.alreadySpun} | 🚫 Bad accounts: ${this.stats.badAccounts} | ❌ Errors: ${this.stats.failCount}` });
    this._emit('terminal', { type: 'info', message: `💰 Score won: ${this.stats.totalScoreWon} | ⏱️ ${elapsed}s | ⚡ ${rate.toLocaleString()}/hr` });

    if (this.stats.badAccounts > 0)
      this._emit('terminal', { type: 'warning', message: `⚠️  ${this.stats.badAccounts} bad accounts (wrong password) — delete or regenerate them.` });
    if (this.stats.alreadySpun > 0)
      this._emit('terminal', { type: 'warning', message: `⚠️  ${this.stats.alreadySpun} already spun today — rerun after ${new Date().toLocaleTimeString()} reset.` });
